...
router.get("/orders", async (_req, res) => {
  ...
  try {
    ...
    const statusCol = has("status") ? "o.status" : "NULL";
    ...
    const sqlOrders = `
      SELECT
        ...
        ${statusCol}      AS status,          -- exact DB value
    ...
    for (const o of orders) {
      // Derive a boolean-ish checked flag for UI/logic ONLY.
      const checkedFlag =
        (typeof o.checked_in_raw === "string" && o.checked_in_raw.toLowerCase() === "done") ||
        (typeof o.checked_in_raw === "number" && Number(o.checked_in_raw) === 1) ||
        !!o.checked_in_at;
      ...
      byOrderPk.set(o.id, {
        ...
        checked_in: checkedFlag ? 1 : 0,    // separate signal
        status: o.status ?? null,           // <-- EXACT DB value, no override
...
router.post("/checkin", async (req, res) => {
  ...
  try {
    ...
    // OPTIONAL: refuse check-in if status='pending'
    if (has("status")) {
      const [rows] = await conn.query(
        `SELECT status FROM orders WHERE ${idCol} = ? LIMIT 1`,
        [idValue]
      );
      if (rows?.[0]) {
        const cur = String(rows[0].status || "").toLowerCase();
        if (cur === "pending") {
          return res.status(400).json({ ok: false, error: "PENDING_NOT_ALLOWED" });
        }
      }
    }
...
router.post("/ticket-book", async (req, res) => {
  ...
  const statusLabel = src; // whatever your enum allows, unchanged
