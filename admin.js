...
router.get("/orders", async (_req, res) => {
  ...
  try {
    ...
    const custSelect =
      ...
      (has("customer_email") ? "o.customer_email" : "NULL") +
      " AS customer_email, " +
      (has("customer_phone") ? "o.customer_phone" : "NULL") +
      " AS customer_phone";
    ...
    // detect order_items table + columns
    ...
    for (const o of orders) {
      const checkedFlag =
        (typeof o.checked_in_raw === "string" &&
          o.checked_in_raw.toLowerCase() === "done") ||
      ...
      byOrderPk.set(o.id, {
        ...
        // ✅ expose customer details for frontend
...
router.post("/ticket-book", async (req, res) => {
  ...
  try {
    ...
    const [bookedSeats] = await pool.query(
      `SELECT seat_no
       FROM order_items
       WHERE table_id = ? AND seat_no IN (${placeholders})`,
      [tId, ...seatList]
    );
    ...
    if (bookedSeats.length > 0) {
      return res
        .status(400)
        .json({ ok: false, error: "SEAT_ALREADY_BOOKED" });
    }
  } catch (e) {
    console.error("[ADMIN][ticket-book] availability check error:", e);
  ...
  try {
    ...
    if (has("order_ref")) {
      cols.push("order_ref");
      vals.push(outOrderId);
      qms.push("?");
    } else if (has("order_id")) {
      cols.push("order_id");
      vals.push(outOrderId);
      qms.push("?");
    }
    ...
    if (has("amount")) {
      cols.push("amount");
      vals.push(grossAmount);
      qms.push("?");
    }
    if (has("fee_amount")) {
      cols.push("fee_amount");
      vals.push(feeAmount);
      qms.push("?");
    }
    if (has("net_amount")) {
      cols.push("net_amount");
      vals.push(netAmount);
      qms.push("?");
    }
    if (has("pay_method")) {
      cols.push("pay_method");
      vals.push(payMethod);
      qms.push("?");
    }
    if (has("status")) {
    ...
    const [[hasItemsTable]] = await conn2.query(
    ...
    if (hasItemsTable) {
      ...
      for (const it of items) {
        ...
        const sql = `INSERT INTO order_items (${cols.join(
          ","
        )}) VALUES (${qms.join(",")})`;
...
router.post("/checkin", async (req, res) => {
  ...
  if (!order_ref) {
    return res
      .status(400)
      .json({ ok: false, error: "ORDER_REF_REQUIRED" });
  }

  const conn = await pool.getConnection();
  try {
    // only rows not yet checked-in
    const [order] = await conn.query(
      "SELECT * FROM orders WHERE order_ref = ? AND (checked_in = 0 OR checked_in IS NULL)",
      [order_ref]
    );
    ...
    if (order.length === 0) {
      return res.status(404).json({
        ok: false,
        error: "ORDER_NOT_FOUND_OR_ALREADY_CHECKED_IN",
      });
    }
