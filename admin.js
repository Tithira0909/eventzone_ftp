...
router.post("/checkin", async (req, res) => {
  ...
  try {
    ...
    if (!idCol) return res.status(400).json({ ok: false, error: "NO_ORDER_ID_COLUMN" });

    const setParts = [];
    if (has("checked_in")) {
      const col = orderCols.find(c => c.Field === "checked_in");
      if (col && /^enum/i.test(col.Type)) setParts.push(`checked_in = 'done'`);
      else setParts.push(`checked_in = 1`);
    }
    if (has("checked_in_at")) setParts.push(`checked_in_at = NOW()`);
    if (has("updated_at")) setParts.push(`updated_at = NOW()`);

    if (setParts.length === 0) return res.status(400).json({ ok: false, error: "NO_CHECKIN_COLUMNS" });

    const sql = `UPDATE orders SET ${setParts.join(", ")} WHERE ${idCol} = ?`;
    const [r] = await conn.query(sql, [idValue]);
    res.json({ ok: true, updated: r.affectedRows });
  } catch (e) {
    console.error("[ADMIN][checkin] Error:", e);
    res.status(500).json({ ok: false, error: "CHECKIN_FAILED", sqlMessage: e?.sqlMessage });
  } finally {
    conn.release();
  }
});
    ...
    if (hasItemsTable) {
      ...
      for (const it of items) {
        ...
        if (ih("item_type")) {
          cols.push("item_type");
          qms.push("?");
          vals.push(it.category === "vip" ? "vipTable" : "seat");
        }
