  ...
  try {
    ...
    const sqlOrders = `
      SELECT
        ...
        ${custSelect}
      FROM orders o
      WHERE o.status = 'ticket_book'  -- Filter for ticket_book orders only
      ORDER BY o.id DESC
    `;
...
router.post("/ticket-book", async (req, res) => {
  ...
  try {
    ...
    if (has("net_amount")) { cols.push("net_amount"); vals.push(netAmount); qms.push("?"); }
...
/* =========================================================================
   CHECK-IN ORDER VIA QR CODE SCAN
   ========================================================================= */
router.post("/check-in", async (req, res) => {
  ...
  try {
    ...
    // Check if order exists and is in ticket_book status
    const [order] = await conn.query("SELECT * FROM orders WHERE order_ref = ?", [orderRef]);
    if (!order || order.status !== "ticket_book") {
